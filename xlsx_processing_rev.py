...
def get_initials(filename: str) -> str:
...
def add_service_provider_column(df: pd.DataFrame, provider_initials: str) -> pd.DataFrame:
...
def load_and_clean_sheet(excel_file: str, sheet_name: str, cols_to_drop: List[str]) -> pd.DataFrame:
...
def clean_and_rename_excel_files(dir_path: str, cols_to_drop: List[str], sheets_to_drop: List[str], json_file: str) -> None:
...
def merge_excel_files(dir_path: str, output_file: str) -> None:
    ...
    for file in os.listdir(dir_path):
        ...
        if file.endswith(EXCEL_EXTENSION) and file != output_file:  # Exclude the merge file
            file_path = os.path.join(dir_path, file)
            try:
